...
do_install() {
	...
	case "$lsb_dist" in
		...
		ubuntu|debian|raspbian)
			...
			# To update the version of the docker engine, check for new versions with the command apt-cache madison docker-engine
			$sh_c 'sleep 3; apt-get update; apt-get install -y -q docker-engine=1.12.6-0~ubuntu-trusty'
			)
			echo_docker_as_nonroot
			exit 0
			;;
